...
public class PaymentServiceImpl implements PaymentService {
    ...
    @Override
    public String createTrial(SubscriptionPlan plan, LoginUser user) throws RazorpayException {
        int amountInPaise = 200; //For activating trial 2rs payment
        return getString(plan, user, amountInPaise, true);
    }
    ...
    public String createOrder(SubscriptionPlan plan, LoginUser user) throws RazorpayException {
        ...
        return getString(plan, user, amountInPaise, false);
    }
    ...
    private String getString(SubscriptionPlan plan, LoginUser user, int amountInPaise, boolean trial) throws RazorpayException {
        ...
        savePaymentDetails(order, user, plan, trial);
        return order.get("id");
    }
    ...
    private void savePaymentDetails(Order order, LoginUser user, SubscriptionPlan plan, boolean trial) {
        ...
        if (trial)
            details.setDuration(plan.getTrialDuration());
        else
